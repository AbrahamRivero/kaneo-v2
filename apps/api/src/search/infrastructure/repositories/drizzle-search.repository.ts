...
import type { SearchRepository } from "../../application/ports/search-repository.port";
...
export class DrizzleSearchRepository implements SearchRepository {
	...
	async getAccessibleWorkspaceIds(userId: string): Promise<string[]> {
		...
		return userWorkspaces.map((w) => w.workspaceId).filter(Boolean);
	}
...
export const searchRepository = new DrizzleSearchRepository();
